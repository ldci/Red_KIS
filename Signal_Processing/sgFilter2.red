...
SGFiltering: function [
	...
	while [i <= (slength - nr)] [
    	...
    	while [n < sglength][
    		idx: to integer! (i - nl) + n
    		val: signal/:idx			;--get signal value
	...
	i: 1
	while [i <= nl] [filter/:i: val2 i: i + 1]
	i: round slength - nr
	while [i <= slength] [filter/:i: val2 i: i + 1]
]
...
generateSignal: function [return: [vector!]] [
	...
	img: make image! reduce [imgSize black] 
	canvas2/image: none
	canvas3/image: none
	clear f/text
...
SGfilterSignal: function [serie [vector!] sgFilter [integer!] canvas color ] [
	canvas/image: none
	img: make image! reduce [imgSize black]
	sfilter: make vector! reduce ['float! 64 (length? serie)]
	rcvSGFilter serie sfilter sgFilter
	plot: compose [line-width 1 pen (color) line]
	i: 0
	forall sfilter [append plot as-pair  i sfilter/1  i: i + 1]
...
view win: layout [
	title "Time Series [Red: Savitzky-Golay 1024]"
	button 150 "Generate Serie" [
		...
		f/text: rejoin ["Rendered in "  round/to third tt * 1000 0.01 " ms"] 
	]
	pad 800x0
	button "Quit" [Quit]
	return
	...
	return
	f: field 512
	do [dp1/selected: dp2/selected: 1 signal: generateSignal]	
]
